...
for test_path in ${tests[@]}; do
  ...
  descriptions+=( "awsproviderlint: [https://github.com/terraform-providers/terraform-provider-aws/tree/master/awsproviderlint/passes/${test}](${test})" )
...
for test_path in ${tests[@]}; do
  test=$(basename ${test_path})
  echo "Static check: ${test}"
  descriptions+=( "tfproviderlint: [https://github.com/bflad/tfproviderlint/tree/master/passes/${test}](${test})" )
  filenames+=( "./results/${test}.txt" )
  awsproviderlint -${test} ${AWS_PROVIDER_PATH}/aws &> ${filenames[${#filenames[@]}-1]}
done
